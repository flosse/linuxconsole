...
static int serport_ldisc_open(struct tty_struct *tty)
{
	...
	sprintf(serport->phys, "%s%d/serio0", ttyname, minor(tty->device) - tty->driver.minor.start);
...
/*
 * serport_ldisc_receive() is called by the low level tty driver when character
s
...
#else
	sprintf(name, "%s%d", tty->driver.name, minor(tty->device) - tty->driver.minor_star);
#endif
