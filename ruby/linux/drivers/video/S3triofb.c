...
static struct fb_fix_screeninfo fb_fix __initdata = {
    "S3Trio ", (unsigned long) NULL, 0, FB_TYPE_PACKED_PIXELS, 0,
    FB_VISUAL_PSEUDOCOLOR, 1, 1, 1, 0, (unsigned long) NULL, 0x1000000, 
    FB_ACCEL_S3_TRIO64
};
...
static struct fb_var_screeninfo fb_var __initdata = {
    ...
    0, FB_ACTIVATE_NOW, -1, -1, FB_ACCELF_TEXT, 1, 0, 0, 0, 0, 0, 0,
                            ...
                            u_int transp, struct fb_info *info)
{
    ...
    mem_out8(regno, info->screen_base+0x1008000 + 0x3c8);
    mem_out8((red & 0xff) >> 2, info->screen_base+0x1008000 + 0x3c9);
    mem_out8((green & 0xff) >> 2, info->screen_base+0x1008000 + 0x3c9);
    ...
    unsigned short cmd;
        ...
        pci_device_loc(dp,&bus,&dev);
                ...
                if(t32 == (PCI_DEVICE_ID_S3_TRIO << 16) + PCI_VENDOR_ID_S3) {
                        pcibios_read_config_dword(bus, dev, PCI_BASE_ADDRESS_0,
&t32);
                        pcibios_read_config_dword(bus, dev, PCI_BASE_ADDRESS_1,
&t32);
...
void __init s3triofb_init_of(struct device_node *dp)
{
    ...
    fb_info.screen_base = ioremap(address,64*1024*1024);
    fb_info.fix.smem_start = address;
    fb_info.fix.mmio_start = address+0x1000000;

    s3trio_resetaccel();

    mem_out8(0x30, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(0x2d, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(0x2e, fb_info.screen_base+0x1008000 + 0x03D4);

    mem_out8(0x50, fb_info.screen_base+0x1008000 + 0x03D4);

    /* disable HW cursor */

    mem_out8(0x39, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(0xa0, fb_info.screen_base+0x1008000 + 0x03D5);

    mem_out8(0x45, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(0, fb_info.screen_base+0x1008000 + 0x03D5);

    mem_out8(0x4e, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(0, fb_info.screen_base+0x1008000 + 0x03D5);

    mem_out8(0x4f, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(0, fb_info.screen_base+0x1008000 + 0x03D5);

    /* init HW cursor */

    CursorBase = (u_long *)(fb_info.screen_base + 2*1024*1024 - 0x400);
	for (i = 0; i < 8; i++) {
    ...
    mem_out8(0x4c, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(((2*1024 - 1)&0xf00)>>8, fb_info.screen_base+0x1008000 + 0x03D5);

    mem_out8(0x4d, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8((2*1024 - 1) & 0xff, fb_info.screen_base+0x1008000 + 0x03D5);

    mem_out8(0x45, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_in8(fb_info.screen_base+0x1008000 + 0x03D4);

    mem_out8(0x4a, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(0x80, fb_info.screen_base+0x1008000 + 0x03D5);
    mem_out8(0x80, fb_info.screen_base+0x1008000 + 0x03D5);
    mem_out8(0x80, fb_info.screen_base+0x1008000 + 0x03D5);

    mem_out8(0x4b, fb_info.screen_base+0x1008000 + 0x03D4);
    mem_out8(0x00, fb_info.screen_base+0x1008000 + 0x03D5);
    mem_out8(0x00, fb_info.screen_base+0x1008000 + 0x03D5);
    mem_out8(0x00, fb_info.screen_base+0x1008000 + 0x03D5);
    ...
    memset((char *)fb_info.screen_base, 0, 640*480);

#if 0
    Trio_RectFill(0, 0, 90, 90, 7, 1);
#endif

    disp.var = fb_info.var;
    disp.cmap.start = 0;
    disp.cmap.len = 0;
    disp.cmap.red = disp.cmap.green = disp.cmap.blue = disp.cmap.transp = NULL;
    disp.screen_base = fb_info.screen_base;
    disp.visual = fb_info.fix.visual;
    disp.type = fb_info.fix.type;
    disp.type_aux = fb_info.fix.type_aux;
    disp.ypanstep = 0;
    disp.ywrapstep = 0;
    disp.line_length = fb_info.fix.line_length;
...
static void Trio_WaitQueue(u_short fifo) {
	...
	u_short status;
        ...
        do
        {
		status = mem_in16(fb_info.screen_base + 0x1000000 + 0x9AE8);
	}  while (!(status & fifo));
...
static void Trio_MoveCursor(u_short x, u_short y) {
	...
	mem_out8(0x39, fb_info.screen_base + 0x1008000 + 0x3d4);
	mem_out8(0xa0, fb_info.screen_base + 0x1008000 + 0x3d5);

	mem_out8(0x46, fb_info.screen_base + 0x1008000 + 0x3d4);
	mem_out8((x & 0x0700) >> 8, fb_info.screen_base + 0x1008000 + 0x3d5);
	mem_out8(0x47, fb_info.screen_base + 0x1008000 + 0x3d4);
	mem_out8(x & 0x00ff, fb_info.screen_base + 0x1008000 + 0x3d5);

	mem_out8(0x48, fb_info.screen_base + 0x1008000 + 0x3d4);
	mem_out8((y & 0x0700) >> 8, fb_info.screen_base + 0x1008000 + 0x3d5);
	mem_out8(0x49, fb_info.screen_base + 0x1008000 + 0x3d4);
	mem_out8(y & 0x00ff, fb_info.screen_base + 0x1008000 + 0x3d5);
