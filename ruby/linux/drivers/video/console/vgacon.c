...
static int vgacon_scroll_region(struct vc_data *c, int t, int b, int dir, int lines);
	...
	case CM_DRAW:
		...
		case CUR_UNDERLINE:
			vgacon_set_cursor_size(c->vc_x, 
					c->vc_font.height - (c->vc_font.height < 10 ? 2 : 3),
					c->vc_font.height - (c->vc_font.height < 10 ? 1 : 2));
			break;
		case CUR_TWO_THIRDS:
			vgacon_set_cursor_size(c->vc_x, c->vc_font.height / 3, 
					 c->vc_font.height - (c->vc_font.height < 10 ? 1 : 2));
			break;
		case CUR_LOWER_THIRD:
			vgacon_set_cursor_size(c->vc_x, c->vc_font.height*2/3, 
					 c->vc_font.height - (c->vc_font.height < 10 ? 1 : 2));
			break;
		case CUR_LOWER_HALF:
			vgacon_set_cursor_size(c->vc_x, c->vc_font.height / 2,
					 	c->vc_font.height - (c->vc_font.height < 10 ? 1 : 2));
			break;
		case CUR_NONE:
			...
			break;
          	default:
			vgacon_set_cursor_size(c->vc_x, 1, c->vc_font.height);
			break;
		}
	    break;
    }
...
static int
vgacon_do_font_op(struct vt_struct *vt, char *arg, int set, int ch512)
{
	int i;
	char *charmap;
	int beg;
	...
	if ((set)&&(ch512!=vga_512_chars)) {	/* attribute controller */
		...
		for(i = 0; i < MAX_NR_CONSOLES; i++) {
			...
			if (vc) vc->vc_hi_font_mask = ch512 ? 0x0800 : 0;
		}
		vga_512_chars=ch512;
...
static int vgacon_font_op(struct vc_data *c, struct console_font_op *op)
{
	...
	if (op->op == KD_FONT_OP_SET) {
		if (op->width != 8 || (op->charcount != 256 && op->charcount != 512))
			return -EINVAL;
		rc = vgacon_do_font_op(c->display_fg, op->data, 1, op->charcount == 512);
		...
		if (!op->data) return 0;
		rc = vgacon_do_font_op(c->display_fg, op->data, 0, 0);
	} else
		rc = -ENOSYS;
	return rc;
}
