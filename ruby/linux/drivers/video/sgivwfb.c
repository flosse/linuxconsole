...
static struct fb_fix_screeninfo sgivwfb_fix __initdata = {
	"SGI Vis WS FB", (unsigned long) NULL, 0, FB_TYPE_PACKED_PIXELS,
	0, FB_VISUAL_PSEUDOCOLOR, 0, 0, 0, 0, DBE_REG_PHYS, DBE_REG_SIZE,
	FB_ACCEL_NONE
};
			...
			struct vm_area_struct *vma)
{
	...
	vma->vm_flags |= VM_IO;
	if (remap_page_range
	    (vma->vm_start, offset, size, vma->vm_page_prot))
		return -EAGAIN;
	vma->vm_file = file;
...
int __init sgivwfb_init(void)
{
	...
	fb_info.screen_base =
	    (char *) ioremap_nocache((unsigned long) sgivwfb_mem_phys,
				     sgivwfb_mem_size);
	if (!fb_info.screen_base) {
		printk(KERN_ERR "sgivwfb: couldn't ioremap screen base\n");
