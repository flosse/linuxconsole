...
unsigned char inverse_translate(int m, int ucs)
{
	...
	if ((q = inverse_translations[m]) &&
	    (p1 = q[ucs >> 11]) &&
	    (p2 = p1[(ucs >> 6) & 0x1f]) && (c = p2[ucs & 0x3f]))
		return c;
	else
		return 0;
}
...
static void set_inverse_translations(int m)
{
	...
	for (c = E_TABSZ - 1; c >= 0; c--) {
		...
		if (!(p1 = q[n = unicode >> 11])) {
			p1 = q[n] =
			    kmalloc(32 * sizeof(unsigned char *),
				    GFP_KERNEL);
			if (!p1) {
				clear_inverse_translations(m);
				return;
		...
		if (!(p2 = p1[n = (unicode >> 6) & 0x1f])) {
			p2 = p1[n] =
			    kmalloc(64 * sizeof(unsigned char),
				    GFP_KERNEL);
			if (!p2) {
				clear_inverse_translations(m);
				return;
...
con_insert_unipair(struct uni_pagedir *p, u_short unicode, u_long fontpos)
{
	...
	if (!(p1 = p->uni_pgdir[n = unicode >> 11])) {
		p1 = p->uni_pgdir[n] =
		    kmalloc(32 * sizeof(u16 *), GFP_KERNEL);
		if (!p1)
			return -ENOMEM;
		for (i = 0; i < 32; i++)
			p1[i] = NULL;
					...
					if ((p2 = *(p1++)))
						for (k = 0; k < 64; k++) {
							if (*p2 < MAX_GLYPH
							    && ect++ <
							    ct) {
								__put_user((u_short) ((i << 11) + (j << 6) + k), &list->unicode);
