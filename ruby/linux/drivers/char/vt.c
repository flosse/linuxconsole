...
void add_softcursor(struct vc_data *vc)
{
	...
	i ^= ((type) & 0xff00);
	if ((type & 0x20)
	    && ((softcursor_original & 0x7000) == (i & 0x7000)))
		i ^= 0x7000;
...
{
	if (!IS_VISIBLE || vc->display_fg->vt_blanked
	    || vcmode == KD_GRAPHICS)
		return;
	if (dectcem) {
		if (cons_num == sel_cons)
...
void scroll_region_up(struct vc_data *vc, unsigned int t, unsigned int b,
		      int nr)
{
	unsigned short *d, *s;
...
void scroll_region_down(struct vc_data *vc, unsigned int t, unsigned int b,
			int nr)
{
	unsigned short *s;
	unsigned int step;
...
{
	attr =
	    build_attr(vc, color, intensity, blink, underline,
		       reverse ^ decscnm);
	video_erase_char =
	    (build_attr(vc, color, intensity, 0, 0, decscnm) << 8) | ' ';
}
...
inline void clear_region(struct vc_data *vc, int sx, int sy, int width,
			 int height)
{
...
inline unsigned short *screenpos(struct vc_data *vc, int offset,
				 int viewed)
{
	unsigned short *p;
...
struct vc_data *vc_allocate(unsigned int currcons)
{
	...
	if (vt->kmalloced || !(vt->first_vc == currcons)) {
		screenbuf =
		    (unsigned short *) kmalloc(screenbuf_size, GFP_KERNEL);
		if (!screenbuf) {
			kfree(vc);
			currcons = -ENOMEM;
			return NULL;
...
int vc_disallocate(struct vc_data *vc)
{
	...
	if (vc) {
		...
		if (vt->kmalloced || !(vt->first_vc == cons_num)) {
			...
			free_bootmem((unsigned long) screenbuf,
				     screenbuf_size);
			free_bootmem((unsigned long) vc,
				     sizeof(struct vc_data));
		}
...
int vc_resize(struct vc_data *vc, unsigned int cols, unsigned int lines)
{
	...
	err =
	    resize_screen(vc, new_cols * vc->vc_font.width,
			  new_rows * vc->vc_font.height);
	if (err) {
		resize_screen(vc, old_cols * vc->vc_font.width,
			      old_rows * vc->vc_font.height);
		return err;
	}

	/* scrollback could have been changed by resize_screen */
	newscreen =
	    (unsigned short *) kmalloc(new_screenbuf_size, GFP_USER);
	if (!newscreen) {
		resize_screen(vc, old_cols * vc->vc_font.width,
			      old_rows * vc->vc_font.height);
		return -ENOMEM;
	...
	while (old_screenbuf < old_scr_end) {
		...
		if (rrem)
			scr_memsetw((void *) (new_scr_end + rlth),
				    video_erase_char, rrem);
	}

	if (new_screenbuf_size > old_screenbuf_size)
		scr_memsetw((void *) newscreen, video_erase_char,
			    new_screenbuf_size - old_screenbuf_size);
			...
			const unsigned char *buf, int count)
{
	...
	while (!tty->stopped && count) {
		...
		if (vc_state == 0 && ok) {
			...
			if ((need_wrap || irm) && IS_VISIBLE
			    && sw->con_putc && draw_x >= 0) {
...
{
	return mk_kdev(TTY_MAJOR,
		       c->index ? c->index : admin_vt->fg_console->vc_num);
}
...
void vt_map_input(struct vt_struct *vt)
{
	...
	vty_driver->table =
	    kmalloc(sizeof(struct tty_struct) * MAX_NR_USER_CONSOLES,
		    GFP_KERNEL);
	vty_driver->termios =
	    kmalloc(sizeof(struct termios) * MAX_NR_USER_CONSOLES,
		    GFP_KERNEL);
	vty_driver->termios_locked =
	    kmalloc(sizeof(struct termios) * MAX_NR_USER_CONSOLES,
		    GFP_KERNEL);
...
static void clear_buffer_attributes(struct vc_data *vc)
{
	...
	for (; count > 0; count--, p++) {
		scr_writew((scr_readw(p) & mask) |
			   (video_erase_char & ~mask), p);
	}
}
