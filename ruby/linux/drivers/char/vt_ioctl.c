 ...
 */
void kd_mksound(struct vc_data *vc, unsigned int period,
...
static inline int
do_kdsk_ioctl(int cmd, struct kbentry *user_kbe, int perm,
...
static inline int
do_kbkeycode_ioctl(struct vc_data *vc, int cmd,
		   struct kbkeycode *user_kbkc, int perm)
{
	struct input_handle *handle = vc->display_fg->keyboard;
...
do_kdgkb_ioctl(int cmd, struct kbsentry *user_kdgkb, int perm)
{
	...
	case KDSKBSENT:
		...
		if (delta <= funcbufleft) {	/* it fits in current buf */
			...
			if (first_free > fj) {
				memmove(fnw + (fj - funcbufptr) + delta,
					fj, first_free - fj);
				for (k = j; k < MAX_NR_FUNC; k++)
					if (func_table[k])
						func_table[k] =
...
int con_font_op(struct vc_data *vc, struct console_font_op *op)
{
	...
	case KD_FONT_OP_SET:
		...
		if (!op->data || !op->charcount
		    || !vc->display_fg->vt_sw->con_font_op)
			return -EINVAL;
...
static inline int
do_fontx_ioctl(struct vc_data *vc, int cmd,
	       struct consolefontdesc *user_cfd, int perm)
{
	...
	if (copy_from_user
	    (&cfdarg, user_cfd, sizeof(struct consolefontdesc)))
		return -EFAULT;
...
static inline int
do_unimap_ioctl(struct vc_data *vc, int cmd, struct unimapdesc *user_ud,
		int perm)
{
	     ...
	     unsigned int cmd, unsigned long arg)
{
	...
	case KDKBDREP:
		{
			...
			input_event(handle->dev, EV_REP, REP_PERIOD,
				    (HZ * kbrep.rate) / 1000);
			input_event(handle->dev, EV_REP, REP_DELAY,
				    (HZ * kbrep.delay) / 1000);
			kbrep.rate =
			    (handle->dev->rep[REP_PERIOD] * 1000) / HZ;
			kbrep.delay =
			    (handle->dev->rep[REP_DELAY] * 1000) / HZ;
			if (copy_to_user
			    ((void *) arg, &kbrep,
			     sizeof(struct kbd_repeat)))
				return -EFAULT;
	...
	case KDGKBMETA:
		ucval =
		    (get_kbd_mode(kbd, VC_META) ? K_ESCPREFIX : K_METABIT);
	      setint:
	...
	case KDSETKEYCODE:
		if (!capable(CAP_SYS_TTY_CONFIG))
			perm = 0;
		return do_kbkeycode_ioctl(vc, cmd,
					  (struct kbkeycode *) arg, perm);

	case KDGKBENT:
	case KDSKBENT:
		return do_kdsk_ioctl(cmd, (struct kbentry *) arg, perm,
				     kbd);
	...
	case KDGKBLED:
		ucval =
		    kbd->ledflagstate | (kbd->default_ledflagstate << 4);
		goto setchar;
	...
	case VT_SETMODE:
		{
			...
			if (!perm)
				return -EPERM;
			if (copy_from_user
			    (&tmp, (void *) arg, sizeof(struct vt_mode)))
				return -EFAULT;
	...
	case VT_GETSTATE:
		{
			...
			if (put_user
			    (vc->display_fg->fg_console->vc_num,
			     &vtstat->v_active))
				return -EFAULT;
			for (i = 0, mask = 0; i < MAX_NR_USER_CONSOLES;
			     ++i, mask <<= 1) {
				tmp =
				    find_vc(i + vc->display_fg->first_vc);
	...
	case VT_RESIZE:
		{
			...
			if (!perm)
				return -EPERM;
			i = verify_area(VERIFY_READ, (void *) vtsizes,
	...
	case VT_RESIZEX:
		{
			struct vt_consize *vtconsize =
			    (struct vt_consize *) arg;
	...
	case PIO_FONT:{
			...
			/* Compatibility */
			op.flags =
			    KD_FONT_FLAG_OLD | KD_FONT_FLAG_DONT_RECALC;
			op.width = 8;
			op.height = 0;
			op.charcount = 256;
			op.data = (char *) arg;
	...
	case PIO_FONTX:
	case GIO_FONTX:
		return do_fontx_ioctl(vc, cmd,
				      (struct consolefontdesc *) arg,
				      perm);
	...
	case PIO_UNIMAP:
	case GIO_UNIMAP:
		return do_unimap_ioctl(vc, cmd, (struct unimapdesc *) arg,
				       perm);
...
inline void switch_screen(struct vc_data *new_vc, struct vc_data *old_vc)
{
	...
	if (old_vc->vc_num != new_vc->vc_num) {
		...
		resize_screen(new_vc,
			      new_vc->vc_cols * new_vc->vc_font.width,
...
void change_console(struct vc_data *new_vc, struct vc_data *old_vc)
{
	...
	if (old_vc->vt_mode.mode == VT_PROCESS) {
		 ...
		 * is awry
		 */
		if (kill_proc(old_vc->vt_pid, old_vc->vt_mode.relsig, 1) ==
		    0) {
			/*
 ...
 */
void complete_change_console(struct vc_data *new_vc,
			     struct vc_data *old_vc)
{
	...
	if (new_vc->vt_mode.mode == VT_PROCESS) {
		 ...
		 * is awry
		 */
		if (kill_proc(new_vc->vt_pid, new_vc->vt_mode.acqsig, 1) !=
		    0) {
			/*
