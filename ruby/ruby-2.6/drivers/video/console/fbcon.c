...
#define TAKE_OVER_CONSOLE 127

char vt2fb[FB_MAX]; /* VC count per fb device*/
...
int fbcon_softback_size = 0; /*FIXME*/
...
int __init fb_console_setup(char *this_opt)
{
	...
	while ((options = strsep(&this_opt, ",")) != NULL) {
		...
		if (!strncmp(options, "vt:", 3)) {
			options += 3;
			for(unit = 0; unit < MAX_NR_CONSOLES && *options; unit++) {
				if(!strncmp(options, "toc", 3)) {
					vt2fb[unit] = TAKE_OVER_CONSOLE;
					options += 3;
				}
				else if(*options >= '0' && *options <= '9')
					vt2fb[unit] = simple_strtoul(options, &options, 10);
				if (*options == '.')
					options++;
			}
		}	
 ...
 *	@vt_num: virtual terminal number to map
...
static inline void fbcon_update_attr(u8 *dst, u8 *src, int attribute,
			       struct vc_data *vc)
{
	...
	while (count) {
		...
		if (mod) {
			while (k--) {
				...
				if (attribute) {
					fbcon_update_attr(buf, src, attribute, vc);
					src = buf;
				}
...
static const char *fbcon_startup(struct vt_struct *vt, int init)
{
	struct display *p = &fb_display[vt->first_vc];
	...
	info = (struct fb_info *)vt->data_hook;
	if (!info)	return NULL;
	info->currcon = -1;
...
static void fbcon_init(struct vc_data *vc, int init)
{
	...
	if (info == NULL)
	    return;
	if (vc != vc->display_fg->fg_console || (info->flags & FBINFO_MODULE) ||
	...
	t = &fb_display[vc->display_fg->fg_console->vc_num];
	if (!vc->vc_font.data) {
		...
		if (p->userfont)
			REFCOUNT(p->fontdata)++;
		con_copy_unimap(vc, vc->display_fg->fg_console);
	}
	if (p->userfont)
	 ...
	 */
	if (IS_VISIBLE && info->fbops->fb_set_par)
	...
	if (logo) {
		...
		if (IS_VISIBLE && vc->vc_mode == KD_TEXT) {
	...
	if (vc == vc->display_fg->fg_console && softback_buf) {
			...
			int width)
{
	...
	if (!info->fbops->fb_blank && vc->display_fg->vt_blanked)
		return;
...
static void fbcon_cursor(struct vc_data *vc, int mode)
{
	...
	if (attribute) {
		...
		fbcon_update_attr(dst, src, attribute, vc);
		src = dst;
	}
...
static int fbcon_switch(struct vc_data *vc)
{
	...
	if (logo_shown >= 0) {
		struct vc_data *conp2 = vc->display_fg->vc_cons[logo_shown];

		if (conp2 && conp2->vc_top == logo_lines
...
static int fbcon_set_font(struct vc_data *vc, struct console_font *font, unsigned flags)
{
	...
	for (i = 0; i < MAX_NR_CONSOLES; i++) {
		struct vc_data *tmp = find_vc(i); /*FIXME*/
...
static int fbcon_scroll(struct vc_data *vc, int lines)
{
	...
	if (!info->fbops->fb_blank &&
	    (vc->display_fg->vt_blanked || vc->vc_mode != KD_TEXT
	     || !lines))
		return 0;
	fbcon_cursor(vc, CM_ERASE);
...
static int fbcon_set_origin(struct vc_data *vc)
{
	if (softback_lines && !vc->display_fg->vt_blanked)
		fbcon_scroll(vc, softback_lines);
	return 0;
}
...
{
	struct vc_data *vc = info->display_fg->fg_console;
	struct display *p;
	int rows, cols;
			      ...
			      struct fb_videomode *mode)
{
	...
	for (i = vt->first_vc; i <= vt->first_vc + vt->vc_count; i++) {
		p = &fb_display[i];
		if (!p || !p->mode)
			continue;
...
static int fbcon_fb_registered(struct fb_info *info)
{
	...
	for(unit = 0; unit < num_registered_fb; unit++)
		if (registered_fb[unit] == info)
			break;

	if(vt2fb[unit] == TAKE_OVER_CONSOLE) {
	        admin_vt->data_hook = (void *)info;
		return take_over_console(admin_vt, &fb_con);
	}
	...
	vt->default_mode = vc;
        vt->vt_kmalloced = 1;
        vt->vt_sw = &fb_con;
	vt->data_hook = (void *)info;
        display_desc = vt_map_display(vt, 1, vt2fb[unit]);
        if (!display_desc) {
		kfree(vt);
		kfree(vc);
	        return -ENODEV;
...
int __init fb_console_init(void)
{
	...
	for(unit = 1; unit < FB_MAX; unit++)
		if(!vt2fb[unit])
			vt2fb[unit] = MAX_NR_USER_CONSOLES;

	if (num_registered_fb)
		for(unit = 0; unit < num_registered_fb; unit++)
			fbcon_fb_registered(registered_fb[unit]);
