...
int iforce_init(int fd)
{
/* We send data to the device by packet that are IFORCE_BLK_SIZE bytes wide */
#define IFORCE_BLK_SIZE	10

        /* Init data for AVB Top Shot Pegasus
         * It should work for any I-Force device */
        unsigned char data_avb_top_shot_pegasus[] = {
                ...
                 };
        int len_avb_top_shot_pegasus = 184;

	int i;

        int len = len_avb_top_shot_pegasus;
        unsigned char* data = data_avb_top_shot_pegasus;

        for (i=0; i+IFORCE_BLK_SIZE-1<len; i+=IFORCE_BLK_SIZE) {
                if (write(fd, &(data[i]), IFORCE_BLK_SIZE) != IFORCE_BLK_SIZE)
			return -1;
		/* Don't feed the device too fast */
		usleep(1000);
        }
	/* Write last part */
	assert(len-i>=0);
...
{ "--iforce",		"-ifor",	B38400, CS8,			SERIO_IFORCE,	0x00,	0,	iforce_init },
